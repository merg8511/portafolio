    ...
    return (
        <div className="bg-slate-950 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 border-b border-white/5">
                ...
                <span className="text-xs font-medium text-slate-500 ml-2">API PREVIEW</span>
                {baseUrl && (
                    <span className="text-xs text-slate-600 ml-auto font-mono">{baseUrl}</span>
                )}
            </div>
            ...
            <div className="p-4 space-y-2 font-mono text-sm">
                {displayEndpoints.map((endpoint, index) => (
                    <div key={index} className="flex items-center gap-3">
                        <span
                            className={`px-2 py-0.5 text-xs font-bold rounded border ${methodColors[endpoint.method]}`}
                        >
                            {endpoint.method}
                        </span>
